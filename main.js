...
function processData(openWeatherData) {
  const { name, sys, main, visibility, wind, weather } = openWeatherData;
  const { temp, temp_max, feels_like, humidity, pressure } = main;
  const { speed } = wind;
  const { country } = sys;
...
function createDataCard(value, label) {
...
function renderWeather({ name, country, temp, weather, temp_max, humidity, pressure, feels_like, visibility, windSpeed }) {
